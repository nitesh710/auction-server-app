...
exports.getUser = async (userName) => {
    try {
        ...
        if (result != undefined && result != null && !result.status) {
            ...
            return {statusCode: 400, status: "Failed", message: "User is not found"};
        }
    } catch (e) {
...
exports.addUser = async (userObject) => {
    try {
        ...
        if(result && !result.status) {
            ...
            return {statusCode: 400, status: "Failed", message: "Failed to sign up user"};
        }
    } catch (e) {
...
exports.login = async (userName, password) => {
    try {
        ...
        if(userResponse != undefined && userResponse != null && !userResponse.status) {
            let isPasswordMatch = await bcrypt.compare(password, userResponse.hashPassword);
            if(isPasswordMatch) {
                return {statusCode: 200, status: "Success", message: "User is loggedin successfully"};
            } else {
                return {statusCode: 401, status: "Failed", message: "Username or password is not valid"};
            }
        } else {
            return {statusCode: 401, status: "Failed", message: "Unauthorized access to login"};
        }
    } catch (e) {
        console.log("Error in userServices, login user:", e.message);
        return {statusCode: 500, status: "Failed", message: "Failed to login user"};       
    }
}
